...
// arktype's keyword flipped every one of those. Reproduced here byte for
// byte (including its redundant `\-`/`\.` escapes, kept so this text
// stays diffable against the source it was copied from, not just
// equivalent to it) from zod 3.25.76's `v3/types.js:384` (`emailRegex`,
// case-insensitive), so both lib/validators.ts (booking email) and
// lib/config.ts (HOST_EMAIL) keep accepting and rejecting exactly what
// they did before the migration off Zod (mig#3).
