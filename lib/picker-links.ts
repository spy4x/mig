 ...
 *  Pulled out of `pushUrl` as a pure function so that behaviour is
 *  unit-testable without driving a real `history.pushState` — a
 *  regression here (e.g. `pushUrl` dropping `tz`) is invisible to a
 *  server-rendered-HTML test, since a server render never calls
 *  `pushUrl` at all. */
